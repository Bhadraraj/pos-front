...
const OrderList = () => {
    ...
    const handlePaymentModeChange = async (event, orderId) => {

        try {
            const newStatus = event.target.value;
            const response = await axios.post(
                `${BASE_URL}/api/updatepaymentstatusrestaurant`,
                {
                    orderid: orderId,
                    paymethod: newStatus,
                },
    ...
    const fetchOrders = async (
        ...
        try {
            ...
            if (response.data.status === "success" && Array.isArray(response.data.orders)) {
                const OrdersLst = response.data.orders.filter(order => (order.OrderPaymentMethod === "Restaurant") && (order.OrderPaymentStatus === "NotPaid"));
                        ...
                        {currentOrders.length > 0 ? (
                            currentOrders.map((order, index) => (
                                <tr key={order.OrderID}>
                                    ...
                                    <td className='text-danger'>{order.OrderPaymentStatus}</td>
                        ...
                        <Form>
                            ...
                            <Form.Group controlId="paymentMode" className='mb-4'>
                                ...
                                <Form.Control
                                    ...
                                    name="paymentMode"
                                    onChange={(e) =>
                                        handlePaymentModeChange(e, selectedOrder.OrderID)
                                    }
                                  
                                >
                                    <option disabled selected>Select Payment Mode</option>
                                    <option value="Cash" className="text-danger">
                                        Cash
                                    </option>
                                    <option value="UPI" className="text-success">
                                        UPI
                                    </option>
                                </Form.Control>
                            </Form.Group>

                            <Form.Group controlId="billPayment" className='mb-4'>
                                <Form.Label>Payment Status</Form.Label>
                                <Form.Control
                                    as="select"
                                    name="billPayment"
                                    onChange={(e) =>
                                        handlePaymentStatusChange(e, selectedOrder.OrderID)
                                    }
                               
                                >
                                    <option disabled selected>Select Payment Status</option>
                                    <option value="NotPaid" className="text-danger">
                                        Not Paid
                                    </option>
